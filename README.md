...
- **删除邮件自动清理附件**：删除邮件时自动检查附件并清理 R2/S3 存储桶中的对象。
- **快速回复功能**：新增不带引用的快速回复选项，直接回复发件人，主题自动生成。
- **邮件编辑器重构**：从弹窗迁移至独立 /compose 页面，支持多标签页编辑。
- **登录背景调节**：支持自定义登录背景暗化因子，个性化界面体验。

### 优化改进

- **回复/转发内容保护**：修复从消息页路由到编辑页时内容丢失的问题。
- **邮件列表查询优化**：缩减单次查询数量至 25 条，提升加载速度。
- **编辑器布局改进**：全宽显示、取消侧边距，增大按钮尺寸，改善移动端体验。
- **快速回复增强**：修正底部加载触发器、自动生成主题、优化图标及工具提示。
- **发送后导航优化**：从消息视图发送邮件后正确重定向至收件箱。
- **页面样式统一**：按钮样式与消息页标题栏保持一致。
