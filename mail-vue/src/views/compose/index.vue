...
function initFromStore() {
  ...
  form.sendEmail = composeStore.sendEmail
  form.accountId = composeStore.accountId
  form.name = composeStore.name
  defValue.value = composeStore.content || ''
  form.subject = composeStore.subject
  form.receiveEmail = composeStore.receiveEmail || []
  form.sendType = composeStore.sendType
...
function syncToStore() {
  composeStore.setForm(toRaw(form))
  composeStore.setBackReply(toRaw(backReply))
}
...
function loadFromBackup(backupId) {
  try {
    const data = JSON.parse(sessionStorage.getItem(`compose_backup_${backupId}`))
    if (data) {
      form.sendEmail = data.sendEmail || ''
      form.receiveEmail = data.receiveEmail || []
      form.accountId = data.accountId ?? -1
      form.name = data.name || ''
      form.subject = data.subject || ''
      form.content = data.content || ''
      form.sendType = data.sendType || ''
      form.text = data.text || ''
      form.emailId = data.emailId || 0
      form.attachments = data.attachments || []
      form.draftId = data.draftId ?? null
      defValue.value = data.content || ''
      backReply.receiveEmail = data.receiveEmail || []
      backReply.subject = data.subject || ''
      backReply.content = data.content || ''
      backReply.sendType = data.sendType || ''
      sessionStorage.removeItem(`compose_backup_${backupId}`)
    }
  } catch (e) {
    console.error('Failed to load compose backup:', e)
  }
}
...
onMounted(() => {
  ...
  if (query.from === 'tab' && query.backupId) {
    ...
    const fromPath = sessionStorage.getItem('compose_from') || '/inbox'
    composeStore.setFromPath(fromPath)
    initFromStore()
  }
});
